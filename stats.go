...
func countDaysSinceDate(date time.Time) int {
	...
	for date.Before(now) {
		date = date.Add(time.Hour * 24) //check changes every hour
		days++
...
func fillCommits(email string, path string, commits map[int]int) map[int]int {
	...
	repo, err := git.PlainOpen(path) //open the repository
	if err != nil {
		panic(err)
	}
	...
	iterator, err := repo.Log(&git.LogOptions{From: ref.Hash()}) //return the commit logs of repository
	...
	err = iterator.ForEach(func(c *object.Commit) error {
		...
		//That mean all change in the input email flag
...
func printMonths() {
	week := getBeginningOfDay(time.Now()).Add(-(daysInLastSixMonths * time.Hour * 24)) // To get Time before six moths from now
	month := week.Month()
	fmt.Printf("         ")
	for {
		if week.Month() != month {
